...
def main():
    ...
    with open('output/mi.json', "rb") as read_content:
        mutualInfo=json.load(read_content)
    mutualInfo = [2.04643934, 2.24643934,0.00000000 ,1.64643934 ]
    ...
    for ax, hx, hy, mi, ti in zip(np.ravel(axarr), Xentropies, Yentropies, mutualInfo, titles):
        ...
        ax.add_artist(venX)
        ax.add_artist(venY)  # ax.text(0,0,'I(X;Y)')
        ...
        ax.axis('off')
    # plt.show()
    plt.savefig("output/pt1_venn_diagrams.png")
    plt.savefig("output/f1b.png")
