...
def generateGrowthModels(repFitCoeffs, carryCaps, repFitFun=None, carryCapFun=None):
    ...
    if carryCapFun and repFitFun:
        return lambda pops: np.array(
            [localLogisticGrowth(p, repFitFun(pops, r, k), carryCapFun(pops, r, k)) for p, r, k in
             zip(pops, repFitCoeffs, carryCaps)])
    if repFitFun:
        return lambda pops: np.array(
            [localLogisticGrowth(p, repFitFun(pops, r, k), k) for p, r, k in zip(pops, repFitCoeffs, carryCaps)])
    if carryCapFun:
        return lambda pops: np.array(
            [localLogisticGrowth(p, r, carryCapFun(pops, r, k)) for p, r, k in zip(pops, repFitCoeffs, carryCaps)])
    else:
