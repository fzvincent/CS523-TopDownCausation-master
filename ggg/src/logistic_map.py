...
def storedata(xtrans, M, N, gens, index, metapop):
    ...
    np.savetxt("output/TEdata/MX_" + str(index) + '_' + str(metapop) + ".csv",
...
# sampled populations
def main(x_0, eps_vals_internal, pic=False):
    K = 100.
    N = 1000
    '''
     NOTE: this took a REALLY long time, so I reduced it to 3 for debugging, however, it seems to have had little 
     effect on the outcome, so I kept this for the final version. - Mark
    '''
    metapops = 10  # For part 3
    # metapops = 3  # For part 3
    ...
    for index in range(len(eps_vals_internal)):
        eps = eps_vals_internal[index]
        print(eps)
        for metapop in range(metapops):
            x = np.empty((N, generations))
...
if __name__ == "__main__":
    ...
    x0 = 1
    print("Staring logistic_map process loop, this may take some time")
    # Part 2 calculations (~20 minutes in docker):
    main(x0, eps_vals_pic, pic=True)
    print("First logistic_map calculation complete, starting second calculation")
    # Part 3 calculations (~40 minutes in docker):
    main(x0, eps_vals, pic=False)
    print("logistic_map done")
    sys.exit(0)
