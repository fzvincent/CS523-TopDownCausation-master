...
for eps = 1:epsvals
    for mpop = 1:metapops
        filename = sprintf('output/TEdata/MX_%d_%d.csv', eps-1, mpop-1);
...
csvwrite('output/TD_data.csv',TD_TE)
csvwrite('output/BU_data.csv',BU_TE)
...
function result = calcTE(v1, v2, k)
    ...
    javaaddpath(strcat(pwd,'/libjidt/infodynamics.jar'));
    % Add utilities to the path
    addpath(strcat(pwd,'/libjidt/demos/octave'));
