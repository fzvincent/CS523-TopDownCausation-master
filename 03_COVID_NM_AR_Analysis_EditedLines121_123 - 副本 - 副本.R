...
day1 <- as.Date("2020-05-15")
day2 <- as.Date("2020-10-15")
...
datNM_TX <- datNYT %>% 
  ...
  filter(state == "New Mexico"| state == "Arizona" ) %>% 
...
TX1_bins <- ceiling((max(datNM_TX1$Arizona)-min(datNM_TX1$Arizona))/TX1_bwidth)

  #mutual information calc
NM_TX1_2d <- discretize2d(datNM_TX1$NewMexico, datNM_TX1$Arizona, 
...
#transfer entropy 
TE_NM_TX1 <- transfer_entropy(datNM_TX1$NewMexico, datNM_TX1$Arizona)
TE_NM_TX1
...
TX2_bins <- ceiling((max(datNM_TX2$Arizona)-min(datNM_TX2$Arizona))/TX2_bwidth)

#mutual information calc
NM_TX2_2d <- discretize2d(datNM_TX2$NewMexico, datNM_TX2$Arizona, 
...
MI_NM_TX2
  ...
  #transfer entropy 
TE_NM_TX2 <- transfer_entropy(datNM_TX2$NewMexico, datNM_TX2$Arizona)
TE_NM_TX2
...
datNM_TX %>% 
  pivot_longer(cols = Arizona:"New Mexico", names_to = "State", values_to = "LogCases") %>% 
  filter(Date1 <= day2) %>% 
  ...
  scale_color_manual(values = c("New Mexico" = "red", Arizona ="black"))+
  annotate(geom = "text", x = as.Date("2020-03-15"), y = 50,
           label = "Both states lockdown", hjust = 0, vjust = 1, size = 4)+
  
  annotate(geom = "text", x = as.Date("2020-03-15"), y = 30, size = 4,
           label = "TE: NM -> AZ = 0.1302 with p-value = 0.073\nTE: TX -> NM = 0", hjust = 0, vjust = 1, size =4)+

  annotate(geom = "text", x = as.Date("2020-08-01"), y = 50, 
           label = "Both states lockdown eased", hjust = 0, vjust = 1, size = 4)+
  
  annotate(geom = "text", x = as.Date("2020-08-01"), y = 30, 
           label = "TE: NM -> AZ = 0.0835 with p-value = 0.097\nTE: TX -> NM = 0", hjust = 0, vjust = 1, size = 4)+
